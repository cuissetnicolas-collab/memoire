...
if fichier_entree is not None:
    ...
    for _, r in df.iterrows():
        ...
        # 4️⃣ Commissions distribution & diffusion
    ...
    # ============================
    # Lignes 411 (équilibrage)
    # ============================
    df_temp = pd.DataFrame(ecritures)

    for date_unique in df_temp["Date"].unique():
        total_debit = df_temp[df_temp["Date"] == date_unique]["Débit"].sum()
        total_credit = df_temp[df_temp["Date"] == date_unique]["Crédit"].sum()
        diff = round(total_debit - total_credit, 2)

        if diff > 0:
            add_ligne(compte_client, f"{libelle_base} - Client {date_unique}", 0.0, diff,
                      date_ligne=pd.to_datetime(date_unique, format="%d/%m/%Y"))
        elif diff < 0:
            add_ligne(compte_client, f"{libelle_base} - Client {date_unique}", abs(diff), 0.0,
                      date_ligne=pd.to_datetime(date_unique, format="%d/%m/%Y"))
